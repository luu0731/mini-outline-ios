        ...
        self.vpnManager.saveToPreferences { error in
            ...
            self.vpnManager.loadFromPreferences { error in
                ...
                case .disconnected, .invalid:
                    do {
                        UserDefaults.init(suiteName: appGroup)?.set("114.114.114.114:12345", forKey: config_server_key)
                        UserDefaults.init(suiteName: appGroup)?.set("chacha20-ietf-poly1305", forKey: config_encrypt_key)
                        UserDefaults.init(suiteName: appGroup)?.set("11111111", forKey: config_password_key)
